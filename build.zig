...
pub fn build(b: *std.Build) !void {
    ...
    const imgui = createImgui(b, static_library_options, legacy);
    ...
    if (legacy) {
        ...
        if (b.lazyDependency("wayland-protocols", .{})) |wayland_protocols| {
            for (
                ...
                {
                    const run_wayland_scanner = b.addSystemCommand(&.{ "wayland-scanner", "client-header" });
                    run_wayland_scanner.addFileArg(wayland_protocols.path(input_file));
                    const header_file = run_wayland_scanner.addOutputFileArg(b.fmt("{s}-client-protocol.h", .{output_filename}));
                    tracy_profiler.addIncludePath(header_file.dirname());
                }
                {
                    const run_wayland_scanner = b.addSystemCommand(&.{ "wayland-scanner", "public-code" });
                    run_wayland_scanner.addFileArg(wayland_protocols.path(input_file));
                    const source_file = run_wayland_scanner.addOutputFileArg(b.fmt("{s}.c", .{output_filename}));
                    tracy_profiler.addCSourceFile(.{ .file = source_file });
                }
            }
        }
        tracy_profiler.linkSystemLibrary("wayland-client");
        tracy_profiler.linkSystemLibrary("wayland-cursor");
        tracy_profiler.linkSystemLibrary("wayland-egl");
...
fn createImgui(b: *std.Build, options: std.Build.StaticLibraryOptions, legacy: bool) *std.Build.Step.Compile {
